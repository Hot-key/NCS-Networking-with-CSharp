...
namespace CGD
{
    ...
    public struct NcsBuffer
    {
        // ****************************************************************************
        // Constructor/Destructor) 
        // ----------------------------------------------------------------------------
        public NcsBuffer(byte[] _Buffer, int _Offset = 0, int _Count = 0)
        {
            m_Buffer = _Buffer;
            m_iOffset = _Offset;
            m_iCount = _Count;
        }
        public NcsBuffer(NcsBuffer _Buffer)
        {
            m_Buffer = _Buffer.m_Buffer;
            m_iOffset = _Buffer.m_iOffset;
            m_iCount = _Buffer.m_iCount;
        }
        public NcsBuffer(int _Size)
        {
        ...
        public NcsBuffer clone()
        {
            ...
            // 3) 적용한다.
            return new NcsBuffer(temp, m_iOffset, m_iCount);
        }
        ...
        public static NcsBuffer operator +(NcsBuffer _Lhs, int _Rhs) { return new NcsBuffer(_Lhs.Array, _Lhs.Offset + _Rhs, _Lhs.Count - _Rhs); }
        public static NcsBuffer operator -(NcsBuffer _Lhs, int _Rhs) { return new NcsBuffer(_Lhs.Array, _Lhs.Offset - _Rhs, _Lhs.Count + _Rhs); }
        public static implicit operator ArraySegment<byte>(NcsBuffer _Rhs) { return new ArraySegment<byte>(_Rhs.Array, _Rhs.Offset, _Rhs.Count); }
        public static explicit operator NcsBuffer(ArraySegment<byte> _Rhs) { return new NcsBuffer(_Rhs.Array, _Rhs.Offset, _Rhs.Count); }

        // 3) append(붙이기)
        public void append_skip(int _Amount) { Debug.Assert((m_Buffer.Length - m_iOffset - m_iCount) >= _Amount); m_iCount += _Amount; }
        public void append(NcsBuffer _Data) { _append_buffer(_Data); }
        ...
        public void append(DateTime _Data)
        {
        ...
        private void _append_buffer(NcsBuffer _Data)
        {
        ...
        public void append_gmlstring(string data)
        {
            append(Encoding.UTF8.GetBytes(Convert.ToString(data)));
            append<byte>(0);
        }
        ...
        private DateTime _extract_datetime(long _Ptr, ref long _Offset)
        {
            ...
            uint tempMillisecond = _extract_Primitive<uint>(_Ptr, ref _Offset);
            ...
            tempObject.AddMilliseconds(tempMillisecond);
        ...
        public string extract_gmlstring() { return Encoding.UTF8.GetString(extract<byte[]>()); }
        ...
        public void set_front(NcsBuffer _Data, int _Offset = 0) { Debug.Assert((m_iOffset + m_iCount + _Data.m_iCount) <= m_Buffer.Length); System.Buffer.BlockCopy(_Data.m_Buffer, _Data.m_iOffset, m_Buffer, m_iOffset + _Offset, _Data.m_iCount); }
        ...
        {
            NcsBuffer temp = new NcsBuffer(m_Buffer, m_iOffset + _Offset, 0);
            temp.append(_Data);
        }

        // 4) get_front( 값 읽기)
        public T get_front<T>(int _Offset = 0) { NcsBuffer temp = new NcsBuffer(m_Buffer, m_iOffset + _Offset, m_iCount - _Offset); return temp.extract<T>(); }
        ...
        public void get_front_buffer(NcsBuffer _Data, int _Offset = 0) { Debug.Assert((m_iOffset + m_iCount + _Data.m_iCount) <= m_Buffer.Length); System.Buffer.BlockCopy(m_Buffer, m_iOffset + _Offset, _Data.m_Buffer, _Data.m_iOffset, _Data.m_iCount); }
